...
type VisaToCountryFormProps = {
  formState: VisaToCountryFormState;
  onChange: (field: keyof VisaToCountryFormState, value: string | boolean) => void;
  onSubmit: (e: FormEvent) => void;
};

const visaToCountrySchema = z.object({
...
type Errors = Partial<Record<keyof VisaToCountryFormState, string>>;

export default function VisaToCountryForm({
  formState,
  onChange,
  onSubmit,
}: VisaToCountryFormProps): React.JSX.Element {
  const [errors, setErrors] = useState<Errors>({});

  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    const result = visaToCountrySchema.safeParse(formState);

    if (!result.success) {
      const fieldErrors: Errors = {};
      for (const issue of result.error.issues) {
        const field = issue.path[0] as keyof VisaToCountryFormState;
