...
type TourTypesTableProps = {
  ...
  isLoading: boolean;
  isError: boolean;
  onEdit: (tourType: TourType) => void;
  onDelete: (tourType: TourType) => void;
  onPageChange: (page: number) => void;
  onPageSizeChange: (size: number) => void;
};

export default function TourTypesTable({
  tourTypes,
  page,
  pageSize,
  totalPages,
  ...
  onPageChange,
  onPageSizeChange,
}: TourTypesTableProps): React.JSX.Element {
  if (isLoading) {
    return <p className="text-gray-600 text-sm">Loading tour types...</p>;
  }

  if (isError) {
    return (
      <p className="text-red-600 text-sm">
        Failed to load tour types. Please try again.
      </p>
    );
  }

  if (!tourTypes.length) {
    return <p className="text-gray-600 text-sm">No tour types found.</p>;
  }
        ...
        <table className="min-w-full border border-gray-200 text-sm">
          ...
          <tbody>
            {tourTypes.map((tourType, index) => (
              <tr key={tourType.id} className="hover:bg-gray-50">
