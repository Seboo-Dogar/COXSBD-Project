...
type FlightFeaturedsTableProps = {
  ...
  isLoading: boolean;
  isError: boolean;
  onEdit: (flightFeatured: FlightFeatured) => void;
  onDelete: (flightFeatured: FlightFeatured) => void;
  onToggleStatus: (flightFeatured: FlightFeatured) => void;
  isTogglingStatus: boolean;
  onPageChange: (page: number) => void;
  onPageSizeChange: (size: number) => void;
};

export default function FlightFeaturedsTable({
  flightFeatureds,
  page,
  pageSize,
  totalPages,
  ...
  onPageChange,
  onPageSizeChange,
}: FlightFeaturedsTableProps): React.JSX.Element {
  if (isLoading) {
    return <p className="text-gray-600 text-sm">Loading featured flights...</p>;
  }

  if (isError) {
    return (
      <p className="text-red-600 text-sm">
        Failed to load featured flights. Please try again.
      </p>
    );
  }

  if (!flightFeatureds.length) {
    return <p className="text-gray-600 text-sm">No featured flights found.</p>;
  }
            ...
            {flightFeatureds.map((flightFeatured, index) => (
              <tr key={flightFeatured.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border-b">
                  <button
                    type="button"
                    onClick={() => onToggleStatus(flightFeatured)}
                    disabled={isTogglingStatus}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full border transition ${
                      flightFeatured.status
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
                        flightFeatured.status ? "translate-x-4" : "translate-x-0.5"
                      }`}
                    />
                  </button>
                </td>
                <td className="px-4 py-2 border-b text-right space-x-2">
                  ...
                  <button
                    ...
                    onClick={() => onDelete(flightFeatured)}
                  >
