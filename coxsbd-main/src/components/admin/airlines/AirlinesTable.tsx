...
type AirlinesTableProps = {
  ...
  onToggleStatus: (airline: Airline) => void;
...
export default function AirlinesTable({
  ...
  onPageChange,
  onPageSizeChange,
}: AirlinesTableProps): React.JSX.Element {
  if (isLoading) {
    return <p className="text-gray-600 text-sm">Loading airlines...</p>;
  }

  if (isError) {
    return (
      <p className="text-red-600 text-sm">
        Failed to load airlines. Please try again.
      </p>
    );
  }

  if (!airlines.length) {
    return <p className="text-gray-600 text-sm">No airlines found.</p>;
  }
            ...
            {airlines.map((airline, index) => (
              <tr key={airline.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border-b">
                  <button
                    type="button"
                    onClick={() => onToggleStatus(airline)}
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
                        airline.status ? "translate-x-4" : "translate-x-0.5"
                      }`}
                    />
                  </button>
                </td>
