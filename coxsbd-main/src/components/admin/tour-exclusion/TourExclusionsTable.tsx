...
import type { TourExclusion } from "@/types/TourExclusion";

type TourExclusionsTableProps = {
  ...
  isLoading: boolean;
  isError: boolean;
  onEdit: (tourExclusion: TourExclusion) => void;
  onDelete: (tourExclusion: TourExclusion) => void;
  onPageChange: (page: number) => void;
  onPageSizeChange: (size: number) => void;
};

export default function TourExclusionsTable({
  tourExclusions,
  page,
  pageSize,
  totalPages,
  ...
  onPageChange,
  onPageSizeChange,
}: TourExclusionsTableProps): React.JSX.Element {
  if (isLoading) {
    return <p className="text-gray-600 text-sm">Loading tour exclusions...</p>;
  }

  if (isError) {
    return (
      <p className="text-red-600 text-sm">
        Failed to load tour exclusions. Please try again.
      </p>
    );
  }

  if (!tourExclusions.length) {
    return <p className="text-gray-600 text-sm">No tour exclusions found.</p>;
  }
    ...
    <div className="space-y-4">
      <div className="overflow-x-auto">
        <table className="min-w-full border border-gray-200 text-sm">
          ...
          <tbody>
            {tourExclusions.map((tourExclusion, index) => (
              <tr key={tourExclusion.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border-b text-right space-x-2">
                  ...
                  <button
                    ...
                    onClick={() => onDelete(tourExclusion)}
                  >
